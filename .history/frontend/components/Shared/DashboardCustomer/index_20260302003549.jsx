...
const DashboardCustomer = () => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="w-full px-4 lg:px-6 space-y-6 text-gray-200">
        {/* HEADER */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {summary.map((i, idx) => (
            <Card
              key={idx}
              bordered={false}
              className="bg-[#1E293B] rounded-2xl shadow-xl border border-[#334155]"
            >
              <div className="text-gray-400 text-sm">{i.label}</div>
        ...
        {compare && (
          <Card
            bordered={false}
            className="bg-[#1E293B] rounded-2xl shadow-lg border border-[#334155]"
          >
            <div className="font-semibold text-gray-300">
              So với kỳ trước:{" "}
              <span
                className={
                  compare.percent >= 0 ? "text-green-400" : "text-red-400"
                }
              >
                {compare.percent}%
              </span>
        ...
        <Card
          bordered={false}
          className="bg-[#1E293B] rounded-2xl shadow-2xl border border-[#334155]"
          title={
            <span className="text-white font-semibold">Dòng tiền & Số dư</span>
          }
        >
