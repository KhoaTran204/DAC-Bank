...
const DashboardCustomer = () => {
    ...
    return (
      <div className="bg-white p-3 border rounded-lg shadow text-sm space-y-1">
        ...
        <div className="text-blue-600">
          Tiền vào: +{d.credit.toLocaleString()} VND
        </div>

        <div className="text-red-500">
          Tiền ra: -{d.debit.toLocaleString()} VND
        </div>

        <div className="font-semibold text-gray-700">
          Số dư: {d.balance.toLocaleString()} VND
        </div>

        {d.isPeak && <Tag color="volcano">Biến động lớn</Tag>}
      </div>
    );
  };
  ...
  useEffect(() => {
    ...
    const successList = filterByDate(allTransactions, start, end).filter(
      (t) => t.status === "success"
    );
