...
const DashboardCustomer = () => {
  ...
  useEffect(() => {
    ...
    const fetchCustomer = async () => {
      const httpReq = http();
      const res = await httpReq.post("/api/find-by-account", {
        accountNo: userInfo.accountNo,
      });
      setFinalBalance(res.data?.data?.finalBalance || 0);
    };

    fetchCustomer();
  }, [userInfo?.accountNo]);

  /* ================= FETCH TRANSACTIONS (THIẾU CHỖ NÀY) ================= */
  useEffect(() => {
    if (!userInfo?.accountNo) return;

    const fetchTransactions = async () => {
      const httpReq = http();
      const res = await httpReq.post("/api/transactions/by-account", {
        accountNo: userInfo.accountNo,
      });
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-white p-3 border rounded-lg shadow text-sm">
        ...
        >
          Số tiền: {isDebit ? "-" : "+"}
          {tx.transactionAmount.toLocaleString()} VND
        </div>
      </div>
    );
  };
    ...
    setSummary([
      {
        ...
        value: `${finalBalance.toLocaleString()} VND`,
      },
      {
  ...
  }, [allTransactions, fromDate, toDate, finalBalance]);
